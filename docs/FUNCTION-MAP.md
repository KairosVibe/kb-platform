...
- **M02 实现状态（2026-09-17）**：F-02.01—F-02.09 与补充入口 API-S01/API-S02/API-S03 已实现（`app/services/org_svc.py`、`app/api/routes/org.py`、`app/schemas/org.py`）。**已知未实现（不得视为已完成）**：F-02.06 处理逻辑第 4 条"**取消相关流**"——那需要 `chat_stream`/`chat_request` 参与（M05 范围），当前只做"递增 `identity_revision` 使既有会话身份失效"；F-02.08 的"不静默撤销知识授权"已覆盖三种引用情形——角色被用户引用、知识授权（`knowledge_acl_role`）引用（M04 落地后即闭环，集成测试 `test_acl_reference_blocks_role_deletion` 钉住）、以及"最后一个管理能力"保护。
- **M03 实现状态（2026-09-17 第二次更新）**：**受理与任务生命周期已实现**——F-03.01（单文件受理，幂等键→`upload_item.client_file_id`，同键同载荷 200 复用/异载荷 409）、F-03.02（批量受理，manifest 与 files 严格对齐、单项 SAVEPOINT 隔离失败）、F-03.03（进度查询，`progress` 恒 null）、F-03.05（人工重试：仅 `failed`、保留 `attempts`、revision 防并发）、F-03.06（恢复收敛：过期租约→`retry_wait`/`failed`、旧版本→`superseded`）与 **H14/H15**（租约原语，`app/tasks/lease.py`）。路由：`POST /api/uploads`、`POST /api/upload-batches`、`GET /api/index-tasks/{task_id}`、`POST /api/index-tasks/{task_id}/retry`（F-03.06 为内部调度，**无** HTTP 路由）。**F-03.04 索引流水线已实现（第三次更新）**——`ingest_svc.run_pipeline`：核验租约 → `superseded` 前置判定 → H19 解析 → H20 清洗 → H21 切片（chunk_config.size/overlap）→ H15 续租 → H11 批量向量化 → H16 持久切片与向量写入（数量核对）→ H17 条件激活；所有终态写入**条件于租约**（不持租约者无权写状态），瞬时错误 → `retry_wait`（+1min），永久错误 → `failed`，版本过时 → `superseded`；任务层错误码只存在于代码（`app/core/errors.py::TaskError`），**不写进契约文档**（两层码不得混写）。Milvus 写入承载（`app/providers/vector_store.py`，H16 的外部副作用）：`ensure_collection`（幂等建集合，维度不符=更换向量空间→永久失败）、`upsert_version_chunks`（按 `(unit_id,version)` **先删后插**实现幂等——业务键三元组无法作 Milvus 主键）、`count_version_chunks`（以库内实数核对、**带期望值轮询**——Milvus 默认 Bounded 一致性，刚写入的数据对 query 不保证立即可见，真实链路补验时曾把"尚未可见"误判为"写入不完整"）。解析器版本 `h19.r1`（`app/tasks/parsing.py`）。新增两个 HTTP 409 错误码：`IDEMPOTENCY_CONFLICT`（同键不同载荷）、`TASK_STATE_CONFLICT`（任务状态不允许该操作），依据 API-CONTRACTS §1 的 409 语义。
...
- **M04 实现状态（2026-09-17）**：**F-04.01—F-04.09 与 API-S04 全部已实现**（`app/services/knowledge_svc.py`、`app/api/routes/knowledge.py`、`app/schemas/knowledge.py`）。路由：`GET/PATCH /api/knowledge-units[/{unit_id}]`、`GET /api/knowledge-units/{unit_id}/chunks`、`POST /api/knowledge-units/{unit_id}/versions`、`POST /api/knowledge-units/{unit_id}/chunk-mutations`、`PUT /api/knowledge-units/{unit_id}/enabled`、`DELETE /api/knowledge-units/{unit_id}`、`PUT/GET /api/knowledge-units/{unit_id}/acl`、`GET /api/acl-entities`。新增 HTTP 错误码：`INDEX_NOT_READY`（409，重新启用须 `index_status=indexed`）、`ACL_ENTITY_INVALID`（422，授权对象不存在或已停用）。
  - **数据读权与防枚举**：正文级访问走 H04 `permission.authorize_units`（已实现，引擎内唯一 IO 步骤，`judge`/`filter_units` 保持纯函数）；**无权与不存在同样 404**（API-CONTRACTS §1 防枚举），403 只属于功能权限层。
  - **版本只增不改**：替换文档/切片编辑都生成 `content_version+1` 新版本并登记新索引任务；旧版本切片原地不动；旧任务由 F-03.04 的 `superseded` 前置判定自然收敛。
  - **切片编辑的源文件重存**：编辑后的全文以 `.txt` 写回受控存储作为新版本 `file_key`（run_pipeline 因此改为按**文件扩展名**决定解析格式）——否则重索引会**静默抹掉人工编辑**。新版本切片行在服务层预建（seq 重排 0..n-1），流水线 H16 按 `(unit,version,seq)` 复用稳定 id，且**清理孤儿切片行**（预建行与重新解析边界略有移动时，避免幽灵切片）。
  - **已登记的实现符号**：`knowledge_svc.get_acl`（API-S04 回填）、`knowledge_svc.build_acl_tags`（台账 acl_tags：`global`/`dept:n`/`role:n`/`user:n`，global 优先、各维 ID 升序）；`parsing.estimate_tokens`（§4 H21 的 token 估计器，由私有转公开——F-04.05 服务层预建切片行需要同一口径，避免两处估计器漂移）。
  - **已知未实现（不得视为已完成）**：H18 清理执行器未建——`cleanup_task` 停留在 `queued`，物理清理（文件/向量）待执行器；F-06.07 来源失效属 M06，FAQ 组件未建，`replace_document`/`set_enabled`/`delete_unit` 中的失效调用为**无动作**（FAQ 命中时的逐项复核保证不读陈旧内容）；`delete_unit` 对已删除单元返回既有清理结果（幂等），依赖"最新一条任务"。
...
- **H18 tasks.cleanup_deleted**
  ...
  - **实现（2026-09-18 落地）**：`app/tasks/cleanup.py` 的 `cleanup_deleted`（契约函数）、`claim_cleanup_task`（单条条件 UPDATE 领取；`exclude_ids` 防同轮调度同步连打——瞬时失败回 queued 必须**下次调度**重试）、`run_cleanup_due`（调度入口）、`CleanupLease`（值类型）；向量侧承载 `vector_store.delete_unit_vectors`（单元级过滤删除）与 `vector_store.count_unit_vectors`（期望值轮询核对归零）。**清理口径：只删派生物**（Milvus 向量 + 受控存储文件），MySQL 的 unit/version/chunk 行保留墓碑态——chunk 被 `message_source`（RESTRICT）引用，保留行使历史引用元数据永远可解析（"已发送内容无法撤回"）。瞬时失败回 `queued` 重试（attempts 领取时递增，≤5），耗尽或墓碑缺失 → `failed`。
